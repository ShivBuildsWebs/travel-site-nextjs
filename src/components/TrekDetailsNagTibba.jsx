// File: src/components/TrekDetailsNagTibba.jsx
"use client";
...
// This component receives all the trek data as props from the page.js file
export default function TrekDetailsNagTibba({ trekData, highlights, pickupLocations, addOns, witnessItems, includedItems }) {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  return (
    <article className="relative z-10 flex flex-col items-center justify-start min-h-screen px-4 py-24 text-white">
      <motion.header
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 1 }}
        className="max-w-6xl w-full mx-auto mb-12 text-center"
      >
        <h1 className="text-5xl md:text-7xl font-bold font-playfair mb-6 bg-gradient-to-br from-amber-400 via-yellow-500 to-amber-300 text-transparent bg-clip-text leading-tight md:leading-snug">
          {trekData.title}
        </h1>
        <p className="text-lg md:text-xl text-slate-300 max-w-3xl mx-auto">
          {trekData.subtitle}
        </p>
        <span className="block w-16 h-1 bg-amber-400 mt-5 mx-auto rounded" />
      </motion.header>
      ...
      <motion.section initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }} className="max-w-6xl w-full mx-auto mb-16 px-4 text-left">
        <h2 className="text-3xl md:text-4xl font-semibold text-amber-400 mb-4 flex items-center gap-2"><FaMapMarkerAlt /> Nag Tibba Trek Pickup Locations</h2>
